...
class SLFMPairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1) 选 seed，并用 SeedSelector refine
...
def main():
    ...
    # 微小版：训练社区只取前 50 个，验证 10 个
    train_cids, val_cids, test_cids = train_val_test_split_communities(all_cids, 50, 10, SEED)
    ...
    # 微小版：先用 CPU，避免 CUDA device-side assert
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for z0, z1, node_ids in loader:
            ...
            node_idx = node_ids.to(device).reshape(B * B0)  # (B*B0,)
        ...
        print(f"[Flow][Epoch {epoch}] "
              f"loss={stats['loss']:.6e}, fm={stats['loss_fm']:.6e}, graph={stats['loss_graph']:.6e}")
